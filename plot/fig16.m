...
surfm(double(tomoNA.latitude),double(tomoNA.longitude),double((dvs_int - mean(dvs_int(:)))/mean(dvs_int(:))) *100);
camlight, material dull, lighting Gouraud, shading interp, alpha(1.)
c=colorbar(ax2,'Position',[0.7 0.65 0.015 0.15]); ylabel(c, 'dV_s [%]'); 
hold on, plotm([shorelines.Lat],[shorelines.Lon],'k'), shading interp, colormap(ax2,flipud(rwbcmap))
plotm(plat,plon,'k'),plotm(study_area(:,2),study_area(:,1),'k','LineWidth',2), caxis([-10 10])
...
title(['Depth ',num2str(double(tomoNA.depth(ii))),' km'])
set(gcf,'units','normalized','outerposition',[0.1 0.1 .5 .5])
print('-dpng','-r400','../fig/fig16')
print('-depsc','-painters','-r300','../fig/fig16')