% Figure 13. Empirical covariance function estimation
% based on seismic tomography model
vrick = load('RICK2013');
T1 = vrick.v3d - mean(mean(vrick.v3d,1),2);
sz = size(vrick.v3d);
R1=vrick.z_tomo;

ET=sum(sum(sum(T1(:,:,:))))/prod(sz(:));     %mean
T1=T1-ET; %demean
VT=sum(sum(sum(T1(:,:,:).^2)))/prod(sz(:));  %dispersion
...
iz  = 25;
Ct = 0;
%VT_iz = sum(sum(T1(:,:,iz).^2))/prod(sz(1:2));
%disp([num2str(6371-R1(iz)),' km'])
step = 4;
for id = 2:nbin-1
    disp(id)
    m=0;
...
load CorrRad_40bin
p2=plot(dd,Cr/Cr(1),'-o','LineWidth',1);
load CorrRad_50bin
p3=plot(dd,Cr/Cr(1),'-o','LineWidth',1);
...
%text(200,0.7,'$C(r)=\exp(-\alpha r) \cos(\omega r))$','interpreter','latex')
legend([p1(1),p2(1),p3(1)],{'30','40','50'})
subplot(122),
load CorrAng_85km
p1=plot(dda,Ct,'o-','LineWidth',1); hold on, 
load CorrAng_145km
p2=plot(dda,Ct,'o-','LineWidth',1);
load CorrAng_185km
p3=plot(dda,Ct,'o-','LineWidth',1);
load CorrAng_245km
p4=plot(dda,Ct,'o-','LineWidth',1);
...
%text(0.3,0.7,'$C(\psi)=\sum \rho^n P_n(cos(\psi))$','interpreter','latex')
legend([p1(1),p2(1),p3(1),p4(1)],{'85 km','145 km','185 km', '245 km'})
